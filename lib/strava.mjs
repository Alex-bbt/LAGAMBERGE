...
// Numéro du jour dans l'année (1 = 1er janvier) pour une date "YYYY-MM-DD".
...
/**
 * Calcule les statistiques de la saison (année du défi) à partir des
 * activités Strava : total de km courus, nombre de sorties, dénivelé.
 * Fonction pure (facile à tester). Ne compte que les courses à pied.
 */
export function computeSeasonStats({ activities, tz, year }) {
  let meters = 0;
  let runs = 0;
  let elevation = 0;
  for (const a of activities || []) {
    const type = a.sport_type || a.type;
    if (!RUN_TYPES.includes(type)) continue;
    const y = Number(localDateStr(new Date(a.start_date), tz).slice(0, 4));
    if (y !== year) continue;
    meters += a.distance || 0;
  ...
  return {
    seasonYear: year,
    seasonKm: Math.round(meters / 1000),
    seasonRuns: runs,
    seasonElevation: Math.round(elevation),
  };
}

/**
 * Distance courue par jour (en km) à partir des activités Strava, pour une
 * année donnée. Fonction pure (facile à tester). Ne compte que les courses à
 * pied ; additionne plusieurs sorties d'un même jour. Sert de matière première
 * à la heatmap du carnet de bord.
 *
 * @returns {Object<string, number>} map "YYYY-MM-DD" → km (1 décimale)
 */
export function computeDailyDistances({ activities, tz, year }) {
  const meters = {};
  for (const a of activities || []) {
    const type = a.sport_type || a.type;
    if (!RUN_TYPES.includes(type)) continue;
    const ymd = localDateStr(new Date(a.start_date), tz);
    if (Number(ymd.slice(0, 4)) !== year) continue;
    meters[ymd] = (meters[ymd] || 0) + (a.distance || 0);
  }
  const km = {};
  for (const [ymd, m] of Object.entries(meters)) {
    km[ymd] = Math.round(m / 100) / 10; // mètres → km, arrondi au 100 m
  }
  return km;
}
...
export async function getStreakFromStrava(env) {
  ...
  const seasonYear = Number(startDate.slice(0, 4));
  ...
  // Distance par jour (matière première de la heatmap du carnet de bord).
  const dailyKm = computeDailyDistances({ activities, tz, year: seasonYear });

  // Nombre de jours écoulés dans l'année (pour la moyenne "km par jour").
  const seasonDays = dayOfYear(result.today);

  return { configured: true, minKm, tz, ...result, ...stats, seasonDays, dailyKm };
}
